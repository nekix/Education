...
* `ViewData` - словарь объектов для передачи произвольных данных (в т.ч. в макеты);
...
* `@{ /* Здесь пишется код */ }` - конструкция для написания кода C# прямо в шаблонах RazorPages. Данный код будет выполнен, но не будет записан в ответ клиенту;
