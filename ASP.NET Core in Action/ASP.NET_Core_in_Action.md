...
Желательный порядок компонентов в конвеере:
...
**При добавлении нескольких `[Authorize("")]` должны быть выполнены все**.
...
Рекомендуется использовать готовых проверенных поставщиков идентификационной информации, вместо того, чтобы писать своих.

Основная спецификация: OAuth 2.0. Это протокол авторизации, который делегирует доступ к ресурсу другому сервису, не раскрывая доп. данных, например как личность.

Основной протокол: OIDC.

Основной поток:

1. Основной сервис направляет к поставщику идентификации;

2. Поставщик идентификации идентифицирует пользователя;

3. Поставщик идентификации перенаправляет вас обратно, но включает в URL-адрес код авторизации. Он используется в сочетании с секретом, который знает только поставщик и сервис.

4. Основной сервис получает токен доступа от поставщика идентификации и использует для вызова API поставщика идентификации для получения информации о пользователе.

**Если утверждения в токене не соотвестуют ожидаемым (в основном из-за внешних поставщиков), то можно использовать преобразование утверждений, чтобы сопоставить их заново (либо добавить доп. утверждения, которых не было в исходном токене). `https://learn.microsoft.com/ru-ru/aspnet/core/security/authentication/claims?view=aspnetcore-9.0`**

**Использование нескольких схем аутентификации `https://learn.microsoft.com/ru-ru/aspnet/core/security/authentication/?view=aspnetcore-9.0`**

**Для тестирования jwt токенов можно использовать инструмент командной строки user-jwts**. Он автоматом может активировать User Secrets для хранения материала ключа подписи. `dotnet user-jwts create` и другие команды. Также настраивает сервисы аутентификации на основе JWT-токенов в `appsettings.Development.json`. 

## Глава 26. Мониторинг и устранение ошибок с помощью журналирования

Существуют службы мониторинга Raygun и Sentry для уведомления о проблемах.

Состав фреймворка журналирования:

`ILogger` - интерфейс для записи логов;

`ILoggerProvider` - создает специальный экземпляр `ILogger` в зависимости от поставщика;

`ILoggerFactory` - создает `ILogger`, который обёртывает каждого поставщика и позволяет писать в них всех разом;

Добавление: `builder.logging.AddConsole();`

Очистка: `builder.logging.ClearProviders();`

Генераторы исходного кода для журналирования: `https://learn.microsoft.com/ru-ru/dotnet/core/extensions/logger-message-generator`.

Уровни сообщений журнала:

1. Critical - катастрофические сбои;

2. Error - ошибки и исключения;

3. Warning - непредвиденная ситуация или ошибка, которую можно обработать;

4. Information - обычный поток приложения;

5. Debug - подробная информация, в основном в разработке;

6. Trace - отслеживание очень подробной информации, в т.ч. чувствительных данных.

Категория журнала - представляет собой строку, в которую по соглашению записывается полное имя класса, в котором записано сообщение. `ILogger<T>`

**Строки сообщений журналов лучше всего задавать с заполнителями, а не интерполяцией. Разные поставщики структурного журналирования смогут получать доп. информацию из этого. Порядок заполнителей и аргументов важен.**

Доступна фильтрация сообщений журнала до того, как они будут записаны по:

1. Уровню сообщения журнала;

2. Категории средства ведения журнала;

3. Поставщику журналирования.

Зачастую настройки журналирования включаются в файлы `appsettings.json`. Но также работают как конфигурация, т.е. другие источники могут дополнять/перопределять.

Приоритет выбора правила:

1. Выбрать все правила для данного поставщика. Если нет, то из секции LogLevel;

2. Из выбранных правил выбрать правила с наиболее совпадающим префиксом категории. Если нет, то Default, если она есть;

3. Если выбрано несколько правил, то использовать последнее;

4. Если правила не выбраны, то использовать глобальный мнимальный уровень LogLevel:Default.

Seq выглядит как удобное средство для просмотра журналов (ну или освоить уже Loki).

Области журналирования позволяют связывать одни и те же деннаые с каждым сообщением журнала в своей группе (по факту так и группировать). `using ILogger.BeginScope<T>(T state)` или например `using ILogger.BeginScope(new Dictionary<string, object> { { "CustomValue", 12345 }} );`.

### Задания

1. Добавить ведение журнала только в консоль. Добавить логирование одного из методов действия с помощью генераторов исходного кода.

    **Решение**
    `https://github.com/nekix/Education/tree/main/ASP.NET%20Core%20in%20Action/FrameworksEducation.AspNetCore/FrameworksEducation.AspNetCore/Chapter%2013`

## Глава 27. Публикация и развертывания приложения

Публикация: `dotnet publish`.

Запуск: `dotnet MyApp.dll` или `dotnet exec MyApp.dll`.

Две модели развертывания: зависящее от платформы (RDD), автономное (SCD).

Требования для развёртывания в промышленном окружении:

1. сервер для запуска приложения;

2. средство загрузки приложения на сервер;

**Развертывание на IIS:**

1. Установка ASP.NET Core Windows Hosting Bundle;

2. Установить модуль IIS AspNetCore;

3. Настроить неуправляемый пул (процесс приложения/ий). Режим конвеера интегрированный;

4. Добавить новый веб-сайт в IIS. Указать наш пул;

5. Предоставить разрешеия пулу приложений к папке с приложением (правая кнопка по папке, Security > Edit > Add, ввести IIS AppPool\NetCore).

6. Не забыть добавить (если используется не `IHostBuilder.ConfigureWebHostDefaults()`) `UseIIS()` для внутрипроцессной или `UseIISIntegration()` для внепроцессной модели размещения;

7. Переместить опубликованное приложение в указанную на сайте папку;

8. Добавить и настроить web.config.

**Развертывание в Linux:**

1. Опубликовать приложение;

2. Установить необходимые комоненты на сервер;

3. Скопировать содержимое на сервер;

4. Настроить обратный прокси-сервер (NGINX, Apache, HAProxy);

5. Настроить отдельный диспетчер процессов для своего приложения.

В самом приложении при этом нужно использовать компонент ForwardedHeadersMiddleware (обычно по умолчанию есть) и установить ASPNETCORE_FORWARDEDHEADERS_ENABLED = true.

`https://learn.microsoft.com/ru-ru/aspnet/core/host-and-deploy/linux-nginx?view=aspnetcore-9.0&tabs=linux-ubuntu`

## Глава 28. Добавление протокола HTTPS в приложение

Шпаргалка по HTTPS: `https://cheatsheetseries.owasp.org/cheatsheets/Transport_Layer_Security_Cheat_Sheet.html`

Есть варианты, где обратный прокси сервер обрабатывает HTTPS, а на Kestrel отсылаются HTTP запросы.

Для регистрация сертификата для разработки: `dotnet dev-certs https --trust`.

**В промышленном окружении необходимо указать Kestrel какой сертификат использовать.**

Рекомендуется добавлять заголовок HTTP Strict Transport Security чтобы пользователь использовал HTTPS. Также можно использовать в сочетании с директивой Upgrade-Insecure-Requests в заголовке Content-Securit-Policy (CSP).

**В окружении разработки лучше не активировать HSTS, иначе не получится протестировать HTTP.**

```
builser.Services.AddHsts(options =>
{
...
MaxAge лучше подбирать от меньшего к большему, общепринятое в промышленном окружении - один год (не менее).

HSTS заголовки загружаются первый раз при HTTPS запросе. Но можно использовать предварительную загрузку HSTS:
...
builder.Services.AddHsts(options =>
{
    ...
    options.ExcludedHosts.Add("never-https.com");
});
```

`HttpsRedirectionMiddleware` позволяет принудительно перенаправлять HTTP запросы на HTTPS (307 Temporary Redirect).

## Глава 29. Повышаем безопасность приложения

### CORS

CORS - совместное использование ресурсов между разными источниками.

Используется когда например приложение хочет использовать AJAX запросы для получения данных.

Браузеры автоматически реализуют правила ограничения домена, запрещающие приложениям использовать JavaScript для вызова веб-API в другом месте, если только это не разрешено явно веб-API. Совпадения проверяются по протоколу, домену, порту.

CORS позволяет указать, кто может обращаться к веб-API.

**Не стоит настривать CORS в web-api, если они не нужны (нет доступа с разных доменов).**

Применить CORS глобально: `app.UseCors("AllowSpecialPolicy");`.

Применить точечно: `[EnableCors]`.

Отключить: `[DisableCors]`.

### Защита от межсайтового скриптинга (XSS)** 

Суть атаки в выполнении вредоносного кода в браузере пользователя путем закидывания в нагрузку запроса специально подготовленных скриптов вместо нормального ввода.

Следует не возвращать пользовательский вввод в том виде, в котором он к вам поступил, валидировать ввод на подозрительный символы, использовать Content-Security-Policy (CSP).

Если нужно какие-либо данные вставить в JS, то в Razor Pages можно использовать `JavaScriptEncoder`, а лучше писать значения в атрибуты HTML. Не использовать `@Html.Raw()`.

### Защита от межсайтовой подделки запросов (CSRF)**

Актуально для веб-сайтов или API, которые используют cookie для аутентификации. Работает с небезопасными запросами (например не работает с GET).

Атака с вредоносного сайта, создающего аутинтифицрованный запрос к вашему API от имени пользователя. Злоумышленник может выполнить POST запрос со своего сайта, на который зашел пользователь, на ваш сайт. Браузер автоматом отправит на твой сайт cookie, которые сохранены для вашего сайта в браузере пользователя.

Распространенный подход - синхронизирующий токен. Один токен добавляется в файл cookie пользователя, второй в форму, которую необходимо защитить. Злоумышленник не может подделать эти токены.

**Razor Pages автоматически добавляет эти токены в формы и проряют их. MVC добавляет токены в форму, но не проверяет автоматом, для включения проверки нужно добавить атрибут [ValidateAntiForgeryToken].**

Также можно добавить в cookie SameSite=Lax или строже.

**В web-api нужно токены вручную добавлять на стороне клиента в запрос**. CORS также помогает.

### Атака с открытым перенаправлением

Возникает когда используется callback (например передается в запросе returnUrl для возврата). Можно передать возврат на сайт злоумышленника.

Самый простой способ обойти: проверять что returnUrl принадлежит вашему (вашим) приложениям до того, как перенаправить пользователя. Для проверки что это локальный URL можно использовать `Url.IsLocalUrl()`. Или использовать метод `LocalRedirect()`, который возбуждает исключение, если редирект не на локальный адрес. Или можно предварительно регистрировать и проверять разрешенные для перенаправления адреса.

### Атака с SQL инъекцией

В приложение отправляются вместо реальных данных SQL код для атаки. ORM защищают в большинстве случаев.

Если все-таки нужен ручной запрос, то лучше использовать параметризированные строки. Под капотом сначала подгатавливается запрос, потом подставляются данные, причём они уже не интерпритируются как SQL. А ещё используется специальный протокол для взаимодействия с бэком БД: `https://www.postgresql.org/docs/current/protocol.html`. Для чистого SQL есть аналогичные команды. `https://www.postgresql.org/docs/current/sql-prepare.html`.

### Небезопасные прямые ссылки

Когда в URL вместо параметра id=1 подставляем например 2, 3 и т.д. При этом нет контроля доступа к ресурсам. Авторизация и аутентификация, GUID в идентификторах.

### Защита паролей и данных пользователей

Привлекательные данные для злоумышленников, юридическая ответственность.

Лучший способ - не хранить лишние данные. Либо хранить в сторонней службе аутентификации.

Не хранить пароли пользователей напрямую, криптографические хещи наше всё.

Использовать 2FA.

Не позволять пользователям использовать ненадежные или скомпрометированные пароли.

Пометить cookie как http (чтобы их нельзя было прочитать из JS) и security для отправки через HTTPS.

Не показывать, зарегистрирован пользователь в приложении или нет.

## Глава 30. Создание приложений ASP.NET Core с помощью обобщенного хоста и класса Startup

`IHost` - ядро приложения ASP.NET Core. Содержит HTTP сервер Kestrel и прочие службы для настройки и генерации ответов.
...
Методы `CreateDefaultBuilder` и `ConfigureWebHostDefaults` разделены для возможности создания приложений без обработки HTTP. Например консольные приложения или службы Windows (демоны systemd Linux).

`Startup` и его методы используют reflection для вызова в builder-е. Или можно реализовать `IStartup` или наследоваться от `StartupBase`. Но при этом нельзя будет использовать DI для внедрения сервисов.

Нужна ручная настройка многих сервисов, которые раньше настраивались по умолчанию.

При такой настройке действует множество неявных соглшашений: `http://mng.bz/Oxxw`.

**Можно вместо `CreateDefaultBuilder` вручную всё настроить.**.

### Выбор между обобщенным и минимальных хостингом

Обобщенный хост:

* легаси с обобщенным хостом;

* точный контроль над созданием `IHost`;

* приложение без HTTP.

## Расширенная настройка ASP.NET Core

### Конвеер промежуточного ПО

`Run()` всегда возвращает ответ. После него ничего не выполняется. Полезен для простого промежуточного ПО. Полезен для сложного промежуточного ПО, которое создает дополнительный фреймворк поверх ASP.NET Core.

`Map()` позволяет ветвить конвеер, запрос может пройти только через одну ветвь. Позволяет сделать некоторую маршрутиазацию. Например для конечной точки проверки работоспособности. Также может создавать сразу конеченые точки. Или полезен, если нужно два независимых более мелких приложения внутри одного (независимы только с точки зрения промежуточного ПО). Также отсекает совпадающие элементы пути и хранит их в `PathBase`. Иметь ввиду при маршрутизации далее по конвееру.

`Use` общее назначение. Позволяет ветвить, прерывать, обрабатывать, продолжать выполнение конвеера. Хорошо подходит для например добавления заголовков в ответ.

Также можно создавать свои middleware классы, основанные на соглашениях и reflection (конструктор с `RequestDelegate next` и метод `public async Task Invoke(HttpContext context)`. Позволяет использовать DI в конструкторе и методе (в конструктор Singleton, т.к. сам middleware Singleton, остальное в метод). Или можно `IMiddleware` использовать. Потом вызвать `app.UseMiddleware<HeadersMiddleware>();`. Обычно создают вспомогатиельные методы расширения по соглашению `UseSecurityHeader` (именно Use).

Можно создать свою конечную точку. Для этого берем наш middleware для обработки запроса. Вызываем `(IEndpointRouteBuilder)app.CreateApplicationBuilder().UseMiddleware<MyMiddleware>().Build()` и `app.Map("/my", endpoint);`. А ещё лучше вынести в метод расширения с префиксом `Map`.


### OptionsBuilder и IConfigureOptions

Если нужно получить настройки `IOptions<T>` из внешнего сервиса, то можно выполнить отложенную настройку. Можно использовать `OptionsBuilder<T>`, **который также может обеспечить проверку объектов `IOptions`**. `http://mng.bz/qrjJ`.

```
builder.Services
    .AddOptions<CurrencyOptions>()
    .BindConfiguration("Currencies")
    .Configure(opts => opts.Currencies = new string[] { "GBP", "USD" });
```
В `Configure` можно передать зависимости, которые будут извлечены DI. Сами опции будут созданы при запросе опции. Внедрять можно только сервисы Singleton. Обойти проблему можно `http://mng.bz/7Dve`.

Либо можно использовать интерфейс `IConfugureOptions<T>` для класса конфигуратора. А потом:

```
builder.Services.AddOptions<CurrencyOptions>()
    .BindConfiguration("Currencies");
builder.AddSingleton<IConfigureOptions<CurrencyOptions>, ConfigureCurrencyOptions>();
```

## Глава 32. Создание собственных компонентов MVC и Razor Pages

### Собственный тег-хелпер Razor

Самый простой способ создать тег-хелпер - наследование от базового класса `TagHelper` и переопределение функции `Process()` или `ProcessAsync()`.

Для записи содержимого на страницу необходимо `HtmlEncoder` внедрить в конструктор и ипользовать.

Атрибуты на свойствах `[HtmlAttributeName("add-machine")]` позволяет задавать их значения из разметки Razor.

Для изменения имени тег-хелпера `[HtmlTargetElement("new-tag-helper-name")]`.

Для использования тег-хелпера необходимл в `_ViewImports.cshtml` добавить `@addTagHelper CustomTagHelpers.SystemInfoTagHelper, CustomTagHelpers`. Либо все тег хелперы из определенной сборки `@addTagHelper *, CustomTagHelpers`.

После этого его можно использовать.

### Компоненты представления

Работают независимо от основной страницы Razor. Могут инкапсулировать сложную бизнес-логику.

В отличии от частичных представлений позволяют инкапсулировать не только логику отрисовки, но и бизнес-логику. Можно использовать и внедрение зависимостей.

**При необходимости можно встроить компоненты Blazor в компоненты Razor. Либо можно заменить компонентами Blazor вызовы AJAX на страницах Razor.**

Для вызовыа компонентов представления используется префикс `vc`:
```
<vc:my-recipes number-of-recipes="3">
</vc:my-recipes>
```

Обычно наследуются от `ViewComponent` и реализуют `InvokeAsync()`. Можно передавать аргументы из шаблона. Есть доступ к `HttpContext`. Частичные представления для этих компонентов хранятся отдельно в:

* `Views/Shared/Components/ComponentName/TemplateName`;

* `Pages/Shared/Components/ComponentName/TemplateName`.

Эти классы (компонентов) имеют ограничения:

* должны быть открытыми;

* нельзя использовать фильтры;

* можно использовать макеты, но они не зависят от макета основного представлния Razor;

* изолированы от страниц Razor, на которых отображаются;

### Создание собственных атрибуто валидации

Наследуем класс атрибута от `ValidationAttribute`. Переопределяем метод `IsValid()`. Возвращаем `ValidationResult()` или `ValidationResult.Success`.

Можно использовать `validationContext.ObjectInstance as CurrencyConverterModel` для получения доступа к проверяемой модели верхнего уровня. Может быть полезно если значение свойства зависит от другого свойства.

Можно также добавить валидацию на стороне клиента, но в значительной степени она зависит от JS библиотек на стороне клиента.

Для получения сервисов через DI можно использовать антипаттерн локатор сервисов (через `ValidationContext`).

Можно также использовать другие библиотеки валидации, как например FluentValidation. В них многие моменты проще и удобнее, но валидации модели теперь определена вне класса модели.

## Глава 33. Вызов удаленных API с помощью IHttpClientFactory

Основные проблемы при использовании `HttpClient`:

* исчерпание сокетов. Проблема в том, что HTTP работает так, что для отправки запроса нужно создать соединение с сервером. При этом на открывается из портов от 0 до 65535 и подключается к IP-адресу и порту HTTP-сервера. При удалении `HttpClient` (`Dispose`) сокет не закрывается немедленно. Соединение переходит в состояние `TIME_WAIT` и ожидает некоторое время (в Windows 240 с) прежде чем полностью закрыть сокет.;

* проблема ротации DNS. Если не удалять `HttpClient`, то он не обнаружит изменения DNS, т.к. соединение не сбрасывается. Всегда будет использоваться старое соединение (если сервер его не закроет принудительно);

`HttpClient` отвечает за оркестрацию запросов, но не за создание самого соединения. Вместо этого вызывается конвеер `HttpMessageHandler`, в конце которого находится `HttpClientHandler`, который устанавливает фактическое соединение и отправляет HTTP-запрос. По принципу конвеера промежуточного ПО. Проблемы сокетов и DNS связаны с удалением `HttpClientHandler` в конце конвеера. А `IHttpClientFactory` отделяет жизненный цикл `HttpClient` от используемого `HttpClientHandler`:

* создается пул доступных обработчиков. `IHttpClientFactory` поддерживает активный обработчик, который используется для создания всех экземпляров `HttpClient` в течении двух минут. При удалении `HttpClient` базовый обработчик не удаляется.

* периодически удаляет (освобождает) обработчики. Каждые 2 минуты создается новый `HttpClientHandler`, который используется для вновь созданных `HttpClient`. Они создают новые соединения и изменения DNS учитываются. А старые обработчики периодически удаляет, если они не используются `HttpClient`.

**Поэтому нет необходимости удалять HttpClient, созданый через IHttpClientFactory`.**

**Если нельзя использовать DI, то используем `IHttpClientFactory` с активированным пулом соединений. `http://mng.bz/E27q`.**

Можно настроить именованные клиенты.

Можно создавать типизированные клиенты.

Можно добавить свой обработчик в конвеер:

* Создать производый от `DelegatingHandler` класс;

* Переопределить `SendAsync` метод. В нём вызвать `base.SendAsync()` для выполнения оставшейся части конвеера;

* Зарегистрировать обработчик в DI;

* Добавить обработчик к именованным или типизированным клиентам `AddHttpMessageHandler`;

## Глава 34. Создание фоновых задач и сервисов

В ASP.NET Core можно использовать `IHostedService` для выполнения задач в фоновом режиме. Сам Kestrel работает как `IHostedService`.

**Пока задача не вернёт `Task` из `StartAsync` приложение не будет продолжать работу.**

Для фоновых задач можно использовать `BackgroundService`, который реализует интерфейс `IHostedService`.

Фоновые задачи Singleton, сответсвенно внедрять в них можно только Singleton сервисы. Но при этом можно создать `IServiceScope` и в нём локатором получать Scope сервисы.

Если не нужно использовать ASP.NET Core, то можно применять `IHost` (сервис рабочей роли).

**В Visual Studio есть шаблон Worker Service как раз сервисов рабочей роли. При этом в проекте задан Sdk="Microsoft.NET.Sdk.Worker."**

Для развертывания в службах Windows используется пакет `Microsoft.Extensions.Hosting.WindowsServices` и `UseWindowsService()` метод.

Для развертывания в виде приложения `systemd` Linux используется пакет `Microsoft.Extensions.Hosting.Systemd` и `UseSystemd()` метод.

После публикации приложения его можно установить как службу: `sc create "My test service" BinPath="C:\path\to\MyService.exe"`.

**Точно также в качестве службы можно установить и приложение ASP.NET Core.**

## Глава 35. Тестирование приложений с помощью xUnit. Глава 36. Тестирование приложений ASP.NET Core.

Ключевая команда: `dotnet test`.

Модульное тестирование - простые небольшие изолированные тесты.

Интеграционное тестирование - тестирование компонентов приложения вместе (либо каких-либо срезов приложения).

Тестирование пользовательского интерфейса - тестирование взаимодействия пользователей с интерфейсом.

Для улучшения опыта тестировани (и разработки) следует:

* избегать статических типов;

* использовать интерфейсы вместо конкретных реализаций;

* реализовывать модульную архитектуру, при которой можно разные модули тестировать независимо.

Для интеграционных тестов ASP.NET Core можно использовать пакет TestHost для запуска внутрипроцессного сервера ASP.NET Core. Также можно использовать поставщиков баз данных в памяти (SQLite).

**Обычно по соглашению для проектов ASP.NET Core .sln находится в корневой папке, основные проекты в src каталоге, а тестовые в test или tests.

При написании тестов типичных подходом является написание тестов для:
...
По хорошему чем более точечные тесты (чем меньше число проверямого за один тест), тем легче отследить и понять в чём была ошибка.

Промежуточное ПО ASP.NET Core тяжело тестировать, т.к. класс HttpContext содержит очень много данных и вариантов их сочетаний. Поэтому их тестирование нежелательно. Пример теста:

```
public async Task ForNonMatchingRequests_CallsNextDelegate()
{
    var context = new DefaultHttpContext();
    context.Request.Path = "/somethingelese";
...
Желательно выносить всю логику из контроллеров, чтобы их было легче протестировать (или вообще чтобы нечего было тестировать), читать код и управлять их поведением. Но из-за их зависимоси от предобработки запроса (впринципе инфраструктуры) не рекомендуется проводить их модульное тестирование.
...
Пакет Test Host использует `HostBuilder`, но конфигурирует приложение не на прослушку сетевых запросов, а но прослушку запросов в памяти. Предоставляет `HttpClient` для отправки запросов в тестовое приложение.
...
Если нужно запускать интеграционны тесты на основе существующего приложения, можно использовать `Microsoft.AspNetCore.Mvc.Testing`. Класс `WebApplicationFactory` для запуска версии реального приложения в памяти. В нём также можно получить `HttpClient`. также можно заменить сервисы для тестов в `_fixture.WithWebHostBuilder ... host.builder.ConfigureTestServices`. Также можно создать свой `WebApplicationFactory` класс для разовой настройки и переиспользования во всех тестах `IClassFixture<CustomWebApplicationFactory>`.

### Задания

1. Добавить интеграционный тест контроллера с созданием полной конфигурации приложения в тестах.

    **Решение**
    `https://github.com/nekix/Education/tree/main/ASP.NET%20Core%20in%20Action/FrameworksEducation.AspNetCore/FrameworksEducation.AspNetCore.Tests/Chapter%2013/WeatherForecast_Tests.cs`
...
30) Уязвимость IDOR `https://owasp.org/www-project-web-security-testing-guide/latest/4-Web_Application_Security_Testing/05-Authorization_Testing/04-Testing_for_Insecure_Direct_Object_References`.

31) Шаблоны архитектуры `https://learn.microsoft.com/ru-ru/azure/architecture/patterns/backends-for-frontends`

32) Изучить OAuth 2.0., OpenID `https://docs.duendesoftware.com/identityserver/quickstarts/1-client-credentials`.

33) RFC7636 - PKCE, расширение кода авторизации, обеспечивает доп. защиту при OAuth 2.0 от потенциальной уязвимости.

34) Паттерн Backend for Frontend (BFF). `https://docs.duendesoftware.com/bff` `https://github.com/damienbod/Blazor.BFF.OpenIDConnect.Template`

35) Какие атаки может предотвратить HSTS `http://mng.bz/5wwa`

36) Ресурсы по безопасности web-приложений: `www.owasp.org`, `www.troyhunt.com`.

37) Подробнее о XSS атаках: `https://owasp.org/www-community/attacks/xss/`. Также о CSP для снижения вероятности атаки: `http://mng.bz/nWW2`. Либа: `http://mng.bz/vnn4`.

38) Защита от XSRF/CSRF в документации ASP.NET Core: `http://mng.bz/QPPv`.

39) Раздел по настройке защиты данных: `http://mng.bz/d40i`. Настройка провайдера хранилища ключей: `https://learn.microsoft.com/en-us/aspnet/core/security/data-protection/implementation/key-storage-providers?view=aspnetcore-5.0&tabs=visual-studio`

40) Account enumeration (проверка зарегистрированных аккаунтов злоумышленником): `https://www.troyhunt.com/understanding-account-enumeration-the-video-tutorial-edition/`.

41) Запуск нескольких независимых конвееров (приложений) ASP.NET Core в одном приложении `http://mng.bz/vzA4`.

42) Проверка заголовков безопасности своего приложения и того, что нужно добавить: `https://securityheaders.com`.

43) Пакет nuget для удобного добавления заголовков безопасности в приложение: `https://github.com/andrewlock/NetEscapades.AspNetCore.SecurityHeaders`.

44) Лучшие практики по производительности ASP.NET Core: `http://mng.bz/KM7X`.