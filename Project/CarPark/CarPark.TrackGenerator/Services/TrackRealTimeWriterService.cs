...
public class TrackRealTimeWriterService
{
    ...
    public TrackRealTimeWriterService(
    ...
    public async Task GenerateAndWriteTrackAsync(int vehicleId, TrackGenerationOptions options, int updateIntervalSeconds)
    {
        ...
        _logger.LogInformation("Начало записи: {StartTime}", startTime);
        _logger.LogInformation("Окончание записи: {EndTime}", endTime);
        _logger.LogInformation("Интервал проверки: {UpdateInterval} секунд", updateIntervalSeconds);

        // 3. Основной цикл записи
        using PeriodicTimer timer = new PeriodicTimer(TimeSpan.FromSeconds(updateIntervalSeconds));
        
        while (await timer.WaitForNextTickAsync())
        {
            DateTimeOffset currentTime = DateTimeOffset.UtcNow;
            
            // Находим точки, которые пора записать
            ...
            {
                // Показываем, что ждем
                TimeSpan timeToNext = track.Points
                    .Skip(processedPoints)
                    .FirstOrDefault()?.Timestamp.Subtract(currentTime) ?? TimeSpan.Zero;
                
                if (timeToNext > TimeSpan.Zero)
                {
                    _logger.LogDebug("Ожидание следующей точки... (через {TimeToNext:F0} сек)", timeToNext.TotalSeconds);
                }
            }
