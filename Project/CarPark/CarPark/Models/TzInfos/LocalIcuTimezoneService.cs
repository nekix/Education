...
namespace CarPark.Models.TzInfos;
...
public class LocalIcuTimezoneService : IDisposable
{
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl, CharSet = CharSet.Unicode)]
    private delegate IntPtr ucal_openDelegate(
        IntPtr zoneID,
        int len,
        [MarshalAs(UnmanagedType.LPStr)] string locale,
        int type,
    ...
    private delegate void ucal_closeDelegate(IntPtr cal);
    ...
    public TimeSpan GetTimeZoneOffset(string ianaId, DateTime? dateTime = null)
    {
        ...
        IntPtr zoneIdPtr = Marshal.StringToHGlobalUni(ianaId);
        
        try
        {
            IntPtr cal = CreateCalendar(zoneIdPtr, ianaId.Length, "en_US", UCAL_GREGORIAN);
            if (cal == IntPtr.Zero)
                return TimeSpan.Zero;
    ...
    private IntPtr CreateCalendar(IntPtr zoneId, int zoneIdLength, string locale, int calendarType)
    {
    ...
    private void CloseCalendar(IntPtr cal)
    {
        ucal_closeDelegate method = GetMethod<ucal_closeDelegate>(GetIcuI18NLibHandle(), "ucal_close");
        method(cal);
    }

    private int GetTimeZoneDisplayNameNative(IntPtr cal, int displayType, string locale, IntPtr result, int resultLength, out ErrorCode status)
    {
    ...
    private void SetCalendarTime(IntPtr cal, DateTime dateTime)
    {
    ...
    private int GetCalendarField(IntPtr cal, int field)
    {
    ...
    private IntPtr GetIcuI18NLibHandle()
    {
    ...
    private T GetMethod<T>(IntPtr handle, string methodName, bool missingInMinimal = false) where T : class
    {
    ...
    private string? GetUnicodeString(Func<IntPtr, int, Tuple<ErrorCode, int>> lambda, int initialLength = 255)
    {
        return GetString(lambda, true, initialLength);
    }

    private string? GetString(Func<IntPtr, int, Tuple<ErrorCode, int>> lambda, bool isUnicodeString = false, int initialLength = 255)
    {
